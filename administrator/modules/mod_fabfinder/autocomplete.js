...
var FbAutocomplete = new Class({
	...
	search: function (e) {
		...
		if (v !== this.searchText && v !== '') {
			...
			if (this.cache[v]) {
				...
				window.fireEvent('autocomplete.search.start', this);
				//Fabrik.loader.start(this.getInputElement());
				this.spinner = new Spinner(this.getInputElement(), {'message': 'searching....'});
				if (this.ajax) {
					this.closeMenu();
					this.ajax.cancel();
				}
				var data = {}
				data[this.options.searchName] = v;
	...
	completeAjax: function (r, v) {
		window.fireEvent('autocomplete.search.done', this);
		r = JSON.decode(r);
		this.cache[v] = r;
	...
	positionMenu: function () {
		...
		this.menu.setStyles({ 'left': coords.left, 'top': (coords.top + coords.height) - 1, 'width': this.options.width? this.options.width : coords.width});
	},
	...
	populateMenu: function (data) {
		...
		ul.empty();
		if (data.length === 1) {
			if (this.options.updateField) {
				this.element.value = data[0].value;
			}
			this.fireEvent('selection', [this, data[0].value]);
		}
		for (var i = 0; i < max; i ++) {
			var pair = data[i];
			if (typeOf(pair) === 'object') {
				li = new Element('li', {'data-value': pair.value, 'class': 'unselected'}).set('text', pair.text);
			} else {
				li = new Element('li', {'class': 'unselected'}).set('html', pair);
			}
			li.inject(ul);
	...
	makeSelection: function (e, li) {
		...
		if (li.hasProperty('data-value') && this.options.updateField) {
	...
	closeMenu: function () {
		if (this.shown) {
			window.fireEvent('autocomplete.close', this);
			this.shown = false;
			this.menu.fade('out');
			this.selected = -1;
	...
	openMenu: function () {
		if (!this.shown) {
			window.fireEvent('autocomplete.open', this);
			this.shown = true;
